...
class ReplayBuffer:
    ...
    def reset(self) -> None:
        self.observations = np.zeros((self.buffer_size, self.num_agents, self.num_envs , self.obs_space),dtype=np.float32)
        ...
        self.next_observations = np.zeros((self.buffer_size, self.num_agents, self.num_envs, self.obs_space),dtype=np.float32)
    ...
    def sample(self) -> Tuple[torch.Tensor,torch.Tensor,torch.Tensor,torch.Tensor,torch.Tensor]:
        ...
        s_tensor_list = [s_tensor for s_tensor in torch.tensor(s, dtype=torch.float, device=self.device).transpose(0,1)]
        ...
        s_prime_tensor_list = [s_prime_tensor for s_prime_tensor in torch.tensor(s_prime, dtype=torch.float, device=self.device).transpose(0,1)]
