...
def train(cfgs):
    ...
    save_dir = f"./runs/qmix/{run_name}"
    if not os.path.exists(save_dir):
        os.makedirs(save_dir)
    if use_wandb:
        wandb_run = start_wandb()
        wandb_run.name = f"qmix_{run_name}"
    ...
    envs = VecRware(train_cfgs['num_process'], "rware:rware-tiny-2ag-v1")
    ...
    agent = QMIXAgent(env_cfgs, model_cfgs, train_cfgs)
    ...
    for t in p_bar:
        ...
        if dones[0] == True:
            ...
            if use_wandb:
                ...
                wandb.log(step=t ,data={"epi_time": np.mean([info['episode_time'] for info in infos])})
            train_reward_sum = [0., 0.]
